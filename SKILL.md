...
- `FUSION_OPUS_EFFORT=xhigh ...` — Opus **judge + synthesizer** reasoning effort (default
  `max`; values `low|medium|high|xhigh|max`). `FUSION_OPUS_LEG_EFFORT=...` sets the claude
  **panel-leg** effort independently (default = `FUSION_OPUS_EFFORT`). (These are
...
Top Anthropic leg = Fable 5 while available (since 2026-07-14). If fable stops working on the subscription (e.g. moves to API-only billing), the leg **auto-falls back to Opus** on a fast no-output failure (elapsed ≤ `FUSION_FABLE_FAST_WINDOW`, default 90s — entitlement errors return in seconds; slow failures and timeouts do NOT fall back, they'd double the leg's wall clock) and the journal records the real model used — the top Anthropic voice is never lost. Judges + synth deliberately stay on Opus: the judge is the measuring instrument behind the journal's longitudinal stats.
